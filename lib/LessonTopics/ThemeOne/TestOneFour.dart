...
import 'package:pylearn/LessonTopics/ThemeOne/TestOneFive.dart';

import '../../Utils/realTimeDatabase.dart';
import '../../Utils/testWidget.dart';

class TestFour extends StatefulWidget {
  const TestFour({Key? key}) : super(key: key);

  @override
  _TestFourState createState() => _TestFourState();
}
          ...
          builder: (BuildContext context) {
            return bottomSheetGreen(
              onPressed: () => Navigator.push(context,
                  MaterialPageRoute(builder: (context) => const TestFive())),
            );
          });
    } else {
              ...
              child: Column(
                children: [
                  ...
                  buttonTestWidget(
                    ...
                    text: 'позволяет добавить внутрь строки слеш',
                    onPressed: firstCheck,
                  ),
                  const SizedBox(height: 15),
                  buttonTestWidget(
                    color: isColor2 ? Colors.white : Colors.green,
                    text: 'осуществляет переход на новую строку',
                    onPressed: secondCheck,
                  ),
                  const SizedBox(height: 15),
                  buttonTestWidget(
                    color: isColor3 ? Colors.white : Colors.green,
                    text: 'позволяет добавить внутрь строки двойную кавычку',
                  ...
                  buttonTestWidget(
                    ...
                    text: 'добавляет табуляцию (4 отступа)',
                  ),
                ],
              ),
            ),
