...
import 'TestTwoThree.dart';

class TestTwoTwo extends StatefulWidget {
  const TestTwoTwo({Key? key}) : super(key: key);

  @override
  _TestTwoTwoState createState() => _TestTwoTwoState();
}

class _TestTwoTwoState extends State<TestTwoTwo> {
  bool isColor1 = true;
  bool isColor2 = true;
  bool isColor3 = true;
  bool isColor4 = true;
          ...
          builder: (BuildContext context) {
            return bottomSheetGreen(
              onPressed: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => const TestTwoThree())),
            );
          });
    } else {
        ...
        body: Column(
          children: [
            const textTestQuestion(
              text:
                  'Какой из операторов используется для получения остатка от деления?',
            ),
            Expanded(
              child: Column(
                children: [
